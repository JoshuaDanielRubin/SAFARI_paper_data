...
def process_bam_files(base_alignments_dir, output_base_dir, reference_genome_base):
    ...
    for main_folder in ['chag', 'vin']:
        ...
        for subfolder in os.listdir(alignments_dir):
            ...
            output_dir = os.path.join(output_base_dir, main_folder, subfolder)
            os.makedirs(output_dir, exist_ok=True)
            
            for bam_file in os.listdir(subfolder_path):
                print(bam_file)
                if 'sorted' in bam_file or not bam_file.endswith(".bam"):
                    continue
                ...
                print(f"Processing {bam_file} in {subfolder}...")
                
                reference_genome = os.path.join(reference_genome_base, "H2a2a1.fa")
                reference_name = "generation_0"
                
                full_bam_path = os.path.join(subfolder_path, bam_file)
    ...
    print("Processing complete for all subfolders.")

# Usage
base_alignments_dir = "alignments/"
output_base_dir = os.path.join(base_alignments_dir, "subs/")
reference_genome_base = "."

process_bam_files(base_alignments_dir, output_base_dir, reference_genome_base)
