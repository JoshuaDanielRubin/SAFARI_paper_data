...
# Filter data for samples where the tool is either SAFARI or vg giraffe and damage level is 'None'
filtered_data = data[(data['tool'].isin(['SAFARI', 'vg giraffe'])) & (data['damage_level'] == 'None')]

# Generate both plots
plot_differences(filtered_data, 'sensitivity', 'Difference in Median Sensitivity\n(SAFARI - vg giraffe)', \
'Difference in Median Sensitivity Between SAFARI and vg giraffe\non Non-damaged Samples ('+sys.argv[3]+")", sys.argv[2]+"_sens_diff_none.png", 'upper right')

plot_differences(filtered_data, 'specificity', 'Difference in Median Specificity\n(SAFARI - vg giraffe)', 'Difference in Median Specificity Between SAFARI and vg giraffe\non Non-damaged Samples ('+sys.argv[3]+")", \
 sys.argv[2]+"_spec_diff_none.png", 'upper left')
